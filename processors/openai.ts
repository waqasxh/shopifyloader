...
export async function seoFriendlyTitle(product: any): Promise<any> {
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: "user",
        content: `Convert this provided title "${product.Title}" into SEO Friendly Product Title for an ecommerce store. Return result as well formatted string with no additional enclosing characters`,
      },
    ],
  });
